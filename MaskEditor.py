...
class MaskEditor(QtGui.QMainWindow):
    ...
    def switch_mask(self):
        if hasattr(self,"vb") and hasattr(self,"mask"):
            if self.maskcheckbox.isChecked():
                self.vb.addItem(self.mask)
            else:
                self.vb.removeItem(self.mask)

    def switch_boundary(self):
        if hasattr(self,"vb") and hasattr(self,"mask_boundary"):
    ...
    def update_mask_boundary(self,t_index=None):
        if not hasattr(self,"maskarray_boundary"):
